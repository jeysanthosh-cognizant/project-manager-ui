...
export class TaskComponent implements OnInit {
  ...
  loadEditFormData() {
    ...
    this.taskService.getTaskByTaskId(this.editTaskId).subscribe(
      (responseData: Task) => {
        ...
        if(this.editLoadtask.user!=null){
        ...
        if(this.editLoadtask.project!=null)
        ...
        if (this.editLoadtask.parentTask != null && this.editLoadtask.parentTask != undefined) {
          this.selectedParentTaskId = this.editLoadtask.parentTask.parentId;
          localParentTaskName = this.editLoadtask.parentTask.parentTask;
        }
        
        this.taskForm = new FormGroup({
          taskId: new FormControl(this.editLoadtask.taskId),
          projectName: new FormControl({ value: this.editLoadtask.project==null ? '' :this.editLoadtask.project.project, disabled: true }, Validators.required),
          taskName: new FormControl(this.editLoadtask.task, Validators.required),
          isParent: new FormControl(false),
          ...
          userId: new FormControl({value: this.editLoadtask.user==null ? '': this.editLoadtask.user.userId})
        });
  ...
  addTask() {
    ...
    if (this.taskForm.value.isParent) {
      ...
      var selectedParentTask = this.parentTasks.find(p2 => p2.parentId === this.selectedParentTaskId);
      var parentId = null;
      var parentTask = null;
      if (selectedParentTask != null && selectedParentTask != undefined) {
        parentId = selectedParentTask.parentId;
        parentTask = selectedParentTask.parentTask;
      }
      ...
      let addTask = new Task(
        ...
        'InPrgress',
        new ParentTask(parentId, parentTask),
        new Project(
